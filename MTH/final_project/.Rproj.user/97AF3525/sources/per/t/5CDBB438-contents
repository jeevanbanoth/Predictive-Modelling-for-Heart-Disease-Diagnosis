...
packages <- c("titanic", "dplyr", "tidyverse", "caTools", "caret", "AER", "arm", 
              "Amelia", "modeest", "mice", "MASS", "cowplot", "rmarkdown", "ggplot2", 
              "pROC", "rpart", "rpart.plot", "GGally", "randomForest", "e1071", "ROCR")
load_packages(packages)

# Function to read CSV files
read_csv_files <- function(train_file, test_file) {
...
# Read CSV files
data_files <- read_csv_files("train.csv", "test.csv")
train <- data_files$train
test <- data_files$test

# Function to analyze metadata
analyze_metadata <- function(train_data, test_data) {
  glimpse(train_data)
  glimpse(test_data)
}

# Analyze metadata
analyze_metadata(train, test)
...
# Preprocess data
processed_data <- preprocess_data(train, test)
train <- processed_data$train
test <- processed_data$test

# Function to train logistic regression model
train_logistic_regression <- function(train_data, test_data, formula) {
  titanic_glm <- glm(formula, data = train_data, family = 'binomial')
  summary(titanic_glm)
  # Further processing steps...
}

# Train logistic regression model
logistic_formula <- Survived ~ Sex
train_logistic_regression(train, test, logistic_formula)
...
# Combine all the models and their accuracies into a data frame
model_accuracies <- data.frame(
  Model = c("Logistic Regression", "Decision Tree", "Bayesian Logistic Regression", 
            "Random Forest", "Neural Network"),
  Accuracy = c(acc_logistic1, acc_tree1, acc_bayesian, acc_rf, acc_nnet)
)
